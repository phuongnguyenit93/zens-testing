...
export const getContent = (paramJokeSeen) => {
    let contentNotSeen = jokeContent.filter (each => !paramJokeSeen.includes(each.id))
    let randomIndex = Math.floor(Math.random()*contentNotSeen.length)
    return contentNotSeen[randomIndex]
}
